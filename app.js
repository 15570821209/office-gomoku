...
function init(){initFakeTable();$("nickname").value=localStorage.getItem("gomokuName")||"";const params=new URLSearchParams(location.search),queryRoom=params.get("room"),queryGame=params.get("game"),arcadeGame=["connect4","tictactoe"].includes(queryGame);if(queryRoom&&!arcadeGame){selectedMode="join";$("roomCode").value=queryRoom.toUpperCase().slice(0,6);document.querySelectorAll("[data-mode]").forEach(b=>b.classList.toggle("selected",b.dataset.mode==="join"));$("roomCodeRow").hidden=false;}state.signalPanel=false;render();tick();setInterval(tick,1000);if(!arcadeGame)setTimeout(()=>welcome.showModal(),180);}
init();
